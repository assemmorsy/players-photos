<template>
  <div ref="parentElm" class="position-relative pt-5 parent" v-if="winner">
    <div
      ...
      "
    >
      <img style="height: 40px; width: 40px" :src="svgs[0]" />
      <img class="mx-2" style="height: 40px; width: 40px" :src="svgs[1]" />
      الفائز :
      <span class="winnerName">{{ winner }}</span>
      <img class="mx-3" style="height: 40px; width: 40px" :src="svgs[2]" />
      <img style="height: 40px; width: 40px" :src="svgs[3]" />
    </div>
    <div class="images">
      <transition-group
        appear
...
export default {
  ...
  setup(props) {
    ...
    return { gameImgBeforeEnter, gameImgEnter, parentElm, svgs };
  },
};
</script>
